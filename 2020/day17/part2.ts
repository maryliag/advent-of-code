...
async function getResult(): Promise<number> {
    ...
    return new Promise<number>(resolve => {
        reader.on("line", (l: string) => {
            let values = l.split('');
            // For each cube create a new key with format x,y,z,w (z and w = 0 for the input)
            for (let x = 0; x < values.length; x++) {
                pocket[x + ',' + y + ',0,0'] = values[x];
            }
            y++;
        })
            ...
            for (let yy = -1; yy <= 1; yy++) {
                for (let zz = -1; zz <= 1; zz++) {
                    for (let ww = -1; ww <= 1; ww++) {
                        ...
                        newKey = (Number(coordinates[0]) + xx) + ',' + (Number(coordinates[1]) + yy) + ',' + (Number(coordinates[2]) + zz) + ',' + (Number(coordinates[3]) + ww);
    ...
    for (let xx = -1; xx <= 1; xx++) {
        for (let yy = -1; yy <= 1; yy++) {
            for (let zz = -1; zz <= 1; zz++) {
                for (let ww = -1; ww <= 1; ww++) {
                    if (xx === 0 && yy === 0 && zz === 0 && ww === 0) continue;
                    positions.push((Number(c[0]) + xx) + ',' + (Number(c[1]) + yy) + ',' + (Number(c[2]) + zz) + ',' + (Number(c[3]) + ww));
                }
            }
        }
    }
