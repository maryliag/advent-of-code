...
func getAllPaths(connections map[string][]string, position string, smallCavesVisited []string, visitedTwice bool) [][]string {
	allPaths := [][]string{}
	...
	if isSmallCave(position) {
		if !contains(smallCavesVisited, position) {
	...
	for i := 0; i < len(connections[position]); i++ {
		...
		// of small caves already visited or if is a small cave but is the first time
		// any small cave will be visited twice.
		if !contains(smallCavesVisited, connections[position][i]) || !visitedTwice {
			paths := getAllPaths(connections, connections[position][i], smallCavesVisited, visitedTwice)
...
func main() {
	...
	paths := getAllPaths(connections, "start", []string{}, false)
