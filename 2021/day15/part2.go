...
func getCaveRiskLevels() [][]int {
	...
	return caveMapTimesFive(caveMap)
}

func caveMapTimesFive(caveMap [][]int) [][]int {
	caveMapFive := [][]int{}
	initialYSize := len(caveMap)
	initialXSize := len(caveMap[0])

	for j := 0; j < len(caveMap)*5; j++ {
		caveMapFive = append(caveMapFive, []int{})
		for i := 0; i < len(caveMap[0])*5; i++ {
			blockX := i / initialXSize
			blockY := j / initialYSize
			value := (caveMap[j%initialYSize][i%initialXSize] + blockX + blockY) % 9
			if value == 0 {
				value = 9
			}
			caveMapFive[j] = append(caveMapFive[j], value)
		}
	}
